...
| # | 项 | 出处/依据 | 预期 | 代价 | 状态 |
|---|---|---|---|---|---|
| A1 | MTP `--draft-tokens` 扫 2/4/5 | nopnop9090：K=2 最快，K 越大不越快 | 小幅（±5%） | ~15 分钟 | 待跑 |
| A2 | `--lookup-ngram` 换 N 值 / auto 模式 | 同上：自适应混合投机在重复负载 406 t/s；我们只测过强制 N=16（−7.7%） | ZCode agent 重发历史，属重复负载，可能转正 | ~30 分钟 | 待跑 |
| A3 | KV dtype 速度对照（k8v4 / int8 / nvfp4） | 只测过 PPL（+0.17%），没测过速度；HermiG：E8 相对 INT8 仅 5.7% decode 代价 | 明确最快的 KV 档 | ~30 分钟 | 待跑 |
