...
**校准事件（8192 太低，已上调至 24576）：** 生产问题"3D 鹈鹕骑自行车"（创意代码任务）
烧穿 8192 思考被截断，WebUI 显示 Reasoning Cancelled，答案仓促。教训：8192 只够问答，
创意或复杂任务的合法思考可达 8K+（真实需求未知，因被截断）。24576 = 3 倍余量，
仍能截住 31,360 的失控个例。另：budget 执行是近似的（budget=200 跑出 224 thinking），非硬墙。
...
### v2 在 200K 的验证（生产上限附近）

| 指标 | 129K（v2） | **200K（v2）** |
|---|---|---|
| prompt | 129,376 | **200,546** |
| prefill | 136.8 t/s | **94.8 t/s** |
| decode | 23.8 t/s | **22.9 t/s** |
| MTP 接受 | 65.2% | 55.6% |
| 召回 | PASS | **PASS** |
| TTFT | 15m46s | 35m15s |

结论：① prefill 曲线延续衰减（658→506→252→148→**95**），200K 处降级大规模发生但全程稳定；
② decode 从 23.8 到 22.9 **几乎没掉**——尽管上下文深了 55%，选择密度从 74% 降到 ~48%，
跳过的 tile 翻倍，抵掉了深度增长的成本。这正是 v2 设计要达到的效果。
③ 引擎日志出现 `thinking 73/8,192`——P0 的 thinking-budget  fuse 已在生产二进制生效（73/8192）。
...
来源：2026-09 全网搜索（NInfer 各 fork、白皮书、BITCOS/HadaCore 论文）+ 头脑风暴。
原则：**先便宜后贵，先测速后改 kernel**；每项做完无论正负都记结论（负结论也是结论）。

### A. 便宜的 sweep（纯测速，不改代码）

| # | 项 | 出处/依据 | 预期 | 代价 | 状态 |
|---|---|---|---|---|---|
| A1 | MTP `--draft-tokens` 扫 2/4/5 | nopnop9090：K=2 最快，K 越大不越快 | 小幅（±5%） | ~15 分钟 | ✅ 做完，**保持 3**（见 §二十） |
| A2 | `--lookup-ngram` 换 N 值 / auto 模式 | 同上：自适应混合投机在重复负载 406 t/s；我们只测过强制 N=16（−7.7%） | ZCode agent 重发历史，属重复负载，可能转正 | ~30 分钟 | ✅ 做完，**保持关闭**（见 §二十） |
| A3 | KV dtype 速度对照（k8v4 / int8 / nvfp4） | 只测过 PPL（+0.17%），没测过速度；HermiG：E8 相对 INT8 仅 5.7% decode 代价 | 明确最快的 KV 档 | ~30 分钟 | ✅ 做完，**保持 nvfp4**（见 §二十） |

### B. 中等投入（读码 + 小改）

| # | 项 | 出处/依据 | 预期 | 代价 | 状态 |
|---|---|---|---|---|---|
| B1 | **ring + 前缀复用共存** | 现状互斥（`--no-prefix-reuse` 关 ring 门）；ZCode 每轮重发全部历史，每轮全量重 prefill；上游两者都有，只是没合体 | 多轮 TTFT 数量级下降（agent 场景收益最大） | 读码查互斥原因 + 圈定兼容范围 + 验证，约 1–2 天 | 待调查 |
| B2 | **Hadamard 快路径** | 白皮书自认"decode 单步最大非矩阵开销，纯实现成本、与质量无关、最直接杠杆"；HadaCore 证明 TC 版 1.1–1.4x | decode 稳定 +10% 级，质量零影响 | kernel（sm_86 适配）+ 验证 | 待做 |
| B3 | 自适应检索预算 | `NINFER_KV_RETRIEVE=12288` 是死的；简单问题和 200K 取证不该同预算 | 小，未知 | 小改 + 验证 | 待做 |

### C. 研究级（投入大，先只做第一步）

| # | 项 | 出处/依据 | 预期 | 代价 | 状态 |
|---|---|---|---|---|---|
| C1 | **三元零值跳过**（BITCOS 思路） | 2026-09 论文：真实三元模型零值占比高达 51.5%，bitmap+sign 布局 GPU decode +27% | 若 Bonsai 零密度高，decode 大幅 | **第一步只做离线零密度测量**（便宜，不用 GPU）；密度高才做 kernel | 待测量 |
| C2 | Q8 重转换（embedding/head） | 解锁 `--embedding-q4` / `--lm-head-q6`（+24K 级上下文） | 中 | 重：要源权重 + 转换链 | 待源权重 |

### D. 明确不做的（已排除，有依据）

| 项 | 排除依据 |
|---|---|
| prefill-chunk 调优 | HermiG 实测 1024→2688 速率打平，不是杠杆 |
| 并发 C=2+（单用户提速） | 上游 C=2–4 是多客户端聚合增益；单用户 ZCode 是单流，且 12G 显存撑不起双 lane KV |
| DFlash2 上生产 | 短/中 +22–48% 但 +2.9G 显存，与 160K/256K 无法共存（§4.2 已测） |
| `--lookup-ngram` 强制模式 | 已测：20K 反降 7.7%（§4.2）；只剩 auto 模式值得试（→A2） |
| `--mlp-a8-decode` | decode 抖动 ±25%（§十四） |
| mask-skip 继续深挖 | v2 在 129K +9.2% 已部署；262K 验证（预期+15–25%）做完即结项，除非数据另说 |


## 二十、A1–A3 sweep 结果（三个否定结论）

方法：ring 256K 同基座（MTP3 + 提案头 + vision overlay），逐配置重启引擎；
prefill 取客户端数（大 prompt 下≈真实值），**decode 取引擎自报 5 次中位**；
sanity 全过（`17×23=391`，lookup-16 那次 64 token 全被思考吃掉，属已知现象）。

| 配置 | 4K prefill | 16K prefill | decode 中位（范围） | MTP 接受 | 显存 | 结论 |
|---|---:|---:|---:|---:|---:|---|
| draft-2 | 583.0 | 473.8 | 78.1（72.0–78.5） | 77.1% | 10321 | — |
| draft-3（现状） | 616.3 | 482.3 | 78.9（74.8–89.6） | 61.7% | 10308 | ✅ 保持 |
| draft-4 | 615.3 | 485.4 | 80.1（64.9–84.0） | 54.8% | 10299 | — |
| draft-5 | 625.5 | 486.9 | 73.4（67.8–102.8） | 48.6% | 10287 | ❌ 更差且更抖 |
| lookup-8 | 585.6 | 483.4 | 72.4（69.9–82.6） | 62.2% | 10298 | ❌ |
| lookup-16 | 599.3 | 475.6 | 82.2（73.3–84.3） | 66.3% | 10359 | ➖ 噪声内 |
| lookup-32 | 585.5 | 470.2 | 80.1（78.1–88.5） | 65.8% | 10373 | ➖ 噪声内 |
| kv-k8v4 | 605.7 | 494.7 | 80.9（73.7–88.3） | 65.9% | 11111 | ⚖️ 见下 |
| kv-int8 | **666.7** | **642.8** | 80.9（73.7–88.6） | 65.4% | **11889** | ⚖️ 见下 |

结论：

1. **A1 保持 draft=3。** 2/3/4 的中位（78.1/78.9/80.1）全落在彼此的波动区间内，
   不可区分；接受率随 draft 数下降（77→62→55→49%，符合预期：越靠后的 draft 越难中）；
   draft=5 中位更低且范围最宽（67.8–102.8）。nopnop 的"K=2 最快"在我们这不成立
   （78.1 vs 78.9，噪声内），保持现状。
2. **A2 保持 lookup 关闭。** lookup-16 的 +4.2%（82.2 vs 78.9）落在 session 间方差内
   （同配置跨次测量差 6.5%，见下）；且本负载（数数到 40）**非重复负载**，
   没测到 lookup 的强项。真正的重复负载（ZCode 历史重发）仍未测，留待以后；
   在此之前默认关闭是对的。
3. **A3 保持 nvfp4。** int8 的 prefill 确实快得多（16K 处 +33%），但显存 11889 MiB
   只剩约 400 MiB 余量，ring 256K 生产不可接受；k8v4 快 +2.5% 但多 806 MiB；
   且两者都要重跑 PPL（KV 精度变了，质量必须重验）。nvfp4 是显存-速度-已验证质量
   的平衡点，不动。

方法论注：**同配置跨次测量差 6.5%**（nvfp4 baseline：上次 74.1，这次 draft-3 78.9），
所以 7% 以内的差异一律判噪声。这是本轮最重要的元结论——以后任何"<8% 的提升"
不经复测不得入库。

## 二十一、B1/C1 结论（复用本来就是通的；零值不够做稀疏）

### B1：ring + 前缀复用共存 —— 不用改代码，本来就是通的

65K 长文档双轮实测（生产二进制，ring 256K）：

| 轮 | prompt | 耗时 | 服务端 |
|---|---|---|---|
| turn1 | 64,169 | 4m30.5s（全量 prefill 238.9 t/s） | queue 26.5 ms |
| turn2 | 64,192 | **1.9s** | **cache 64,164（100.0%, turn closure），TTFT 430 ms** |

召回 PARROT-31 ✅。**100.0% 缓存命中**——turn2 根本没重算。

结论重构（相对立项时的假设）：

1. 所谓"互斥"并不存在。设计文档的两条旧缺陷：#5 scratch 溢出**已修**，
   #6 拒请求是**主机区容量**问题（已用配置规避）。加上 T4 短轮 68% turn-closure，
   复用在 ring 里**全深度可用**。
2. 真正的硬规则只有一条：**永远别对 ring 传 `--no-prefix-reuse`**——它会把地址空间
   按 96K 池构建，ring 的逻辑映射直接崩。我们的启动器本来就没传。
3. 所以 ZCode 多轮慢**不是复用没开，而是首轮 prefill 本来就慢**（65K 要 4 分钟）。
   下一步是 prefill 速度（§八曲线），不是复用兼容。

### C1：三元零密度 32.76% —— 稀疏 kernel 不值得做

全量扫描制品全部 323 个 `t2_g128_fp16` 张量（27,540,848,640 个 code，7.32 GiB payload，
numpy-free 标准库实现，`collections.Counter` C 速统计；打包按
`tools/artifact/codecs/row_split.py::_pack_two_bit`，零值 = `0b00`）：

| 类别 | codes | 零值占比 |
|---|---:|---:|
| attn | 503,316,480 | 32.75% |
| embedding | 1,271,398,400 | 32.80% |
| gdn | 1,509,949,440 | 32.76% |
| head | 1,942,487,040 | 32.79% |
| mlp | 5,704,253,440 | 32.77% |
| other | 16,609,443,840 | 32.76% |
| **合计** | **27,540,848,640** | **32.76%** |

结论：

1. **远低于立项依据**（BITCOS 论文称真实模型高达 51.5%；51.5% 下 GPU decode +27%）。
   32.76% 下理论上限约 1.1x，再扣解包开销——**不值得开 kernel 项目**，C1 关闭。
2. 各类别**均匀一致**（32.75–32.80%）——正是 Hadamard 旋转把分布抹平的证据；
   均匀即无结构可剥削，连"只对某类层做稀疏"都无从谈起。
3. 方法副产品：`.ninfer` 容器解析器（magic/JSON/逻辑 payload 偏移）与行分裂解包顺序
   已验证可用（全 K 满足 K%128==0，无填充码污染），复用脚本见评估目录。

附带环境坑：eval venv 的 Python 解析 405KB JSON 报 MemoryError（系统内存 22GB 空闲，
纯属环境 bug）——离线分析直接上系统 Python + 标准库。
