...
void validate_target_options(const execution::Parameters& parameters, DeviceContext& device,
    ...
    // LOCAL PROTOTYPE (KVMem-style ring): the device pool is allowed to be SMALLER than the
    // logical context. The address-space allocator then recycles the oldest logical pages as
    // the sequence grows (see KVAddressSpaceStore::ensure_mapped_to_tokens), and the retrieval
    // mask keeps attention inside the resident window. With kv_capacity == max_context the
    // recycle path never runs and behaviour is bit-identical to the original engine.
    const std::uint32_t minimum_pages = options.max_concurrency;
...
make_sequence_planner_impl(const execution::Parameters& parameters, DeviceContext& device,
    ...
    // LOCAL PROTOTYPE (KVMem-style ring): the device pool may be SMALLER than the logical
    // context. The address-space allocator recycles the oldest logical pages as the sequence
    // grows, and the retrieval mask keeps attention inside the resident window.
    const std::uint32_t minimum_pages = inputs.max_concurrency;
    const std::uint64_t maximum_pages64 =
