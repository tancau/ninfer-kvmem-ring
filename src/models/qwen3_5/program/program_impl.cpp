...
ProgramImpl::ProgramImpl(const execution::Parameters& parameters_in, const SequencePlanImpl& plan,
    ...
    if (plan.context_cache.host_kv_capacity_bytes != 0) {
        ...
        const char* pageable_env = std::getenv("NINFER_HOST_PAGEABLE");
        const bool pageable = pageable_env != nullptr && pageable_env[0] != '\0' && pageable_env[0] != '0';
        if (cudaMemGetInfo(&free_device, &total_device) == cudaSuccess) {
            ...
            // LOCAL PROTOTYPE (KVMem-style ring): with pageable host memory there is no WDDM
            // mapping and therefore no charge against the card, so the arena is limited only by
            // system RAM -- keep a modest device-side reserve and otherwise honour the request.
            // Gated on the environment so the shipped configuration keeps the conservative default.
            // LOCAL PROTOTYPE (KVMem-style ring): the default split is deliberately conservative.
            // When the experiment explicitly asks for a larger Host spill, reserve up to the request
            // while still leaving a fixed headroom. Gated on the environment so the shipped
            // configuration keeps the conservative default.
            const char* overcommit = std::getenv("NINFER_KV_HOST_OVERCOMMIT_MIB");
            if (overcommit != nullptr) {
                const long mib = std::strtol(overcommit, nullptr, 10);
                if (mib > 0) {
                    // LOCAL PROTOTYPE: the conservative default halves the budget after a 1 GiB
                    // floor. When the experiment asks for a specific size, take up to it while
                    // leaving only 512 MiB for the run's own late allocations.
                    const std::size_t headroom = 512ULL << 20;
                    const std::size_t budget =
                        free_device > headroom ? free_device - headroom : static_cast<std::size_t>(0);
                    const std::size_t want = static_cast<std::size_t>(mib) << 20;
                    const std::size_t capped = std::min(want, budget);
                    if (capped > reserved_bytes) {
                        reserved_bytes = capped - (capped % minimum_stride);
                    }
                }
            }
        }
