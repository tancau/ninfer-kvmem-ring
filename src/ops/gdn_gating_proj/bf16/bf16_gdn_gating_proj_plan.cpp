...
Bf16GdnGatingPlan bf16_gdn_gating_resolve_plan(const Bf16GdnGatingProblem& problem) {
    ...
    // --- LOCAL PATCH (small-SM sm_86 devices) ---------------------------------
    // The route table below is calibrated for the smallest device this fork officially
    // supports: an RTX 3090 with 82 SMs. A cooperative route needs its whole grid resident,
    // so on a smaller sm_86 part (an RTX 3060 has 28 SMs) the route the table selects for a
    // given column count can exceed the real residency budget and be rejected outright
    // ("candidate is not legal for exact problem").
    //
    // Pick the table's schedule first, then fall back through progressively cheaper
    // cooperative splits, ending at MmaUnsplit, which is not a cooperative launch and
    // therefore carries no residency constraint at all.
    ...
    if (candidate_is_legal(preferred, problem)) {
        return bf16_gdn_gating_resolve_candidate(preferred, problem);
    }
    for (const Bf16GdnGatingScheduleId schedule : {
    ...
    throw std::logic_error("BF16 GDN gating: admitted problem has no legal route");
}
