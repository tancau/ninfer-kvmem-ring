...
ServeOptions parse_serve_options(int argc, char** argv) {
    ...
    // --max-context, but the storage layer that backs it (host demotion of the recycled pages) is
    // NOT implemented yet, so the combination is refused unless the experiment is explicitly
    // enabled with NINFER_KV_RING=1. This keeps the dense production configuration exactly as it
    // was: without the flag a too-small pool is rejected, just like the original engine.
